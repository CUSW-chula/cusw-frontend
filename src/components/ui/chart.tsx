              ...
              >
                {formatter && item?.value !== undefined && item.name ? (
                  ...
                  <>
                    ...
                    <div
                      className={cn(
                        "flex flex-row justify-between leading-none gap-2",
                        nestLabel ? "items-end" : "items-center"
                      )}
                    >
                      <div className="grid gap-1.5">
                        {nestLabel ? tooltipLabel : null}
                        <span className="text-black font-BaiJamjuree font-medium dark:text-neutral-400 w-20 ">
                      ...
                      {item.value && (    
                        <div className="flex flex-row- font-BaiJamjuree  font-medium tabular-nums ">
                          <>
                          <div className="text-black text-right w-full">
                             {item.value.toLocaleString()}
                          </div>
                           <div className="text-neutral-500 dark:text-neutral-400 ml-3 text-right w-full">
                             {(() => {
                            const p = item.payload;
                            const dataKey =
                              typeof item.dataKey === "string"
                                ? item.dataKey.toLowerCase()
                                : "";
                            if (dataKey === "assigned")
                              return ` ${p.perAssigned}%`;
                            if (dataKey === "inrecheck")
                              return ` ${p.perInRecheck}%`;
                            if (dataKey === "underreview")
                              return ` ${p.perUnderReview ?? 0}%`;
                            if (dataKey === "done") return ` ${p.perDone}%`;
                            return "";
                          })()}
                           </div>
